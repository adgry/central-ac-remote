...
格式就是 `http://` 加 IP，比如 `http://192.168.1.50`。端口和路径都不用写。

**不知道 IP 就点「搜索红外桥」**（添加页和红外调试页各有一个）。它会扫一遍手机所在的
那个网段，桥会在自己的状态 JSON 里报名字，所以搜到就是它，不会认错，搜到直接填好。

想手动填也行，IP 有三个地方能看到：

* 固件刷完后串口（115200）会打印 `就绪，地址 http://192.168.x.x`
* 路由器后台的客户端列表里找 `ESP-xxxxxx` 之类的名字
* 在浏览器里打开 `http://<IP>/`，是桥的话会返回一段 JSON

只填 IP 不写 `http://` 也认，粘贴成 `http://192.168.1.50/send` 也认——都会被自动规整。

建议在路由器上给它绑一个固定 IP，不然重启后地址可能变。
